...
function smartSplit(userId, amountUSD) {
  ...
  // 1. Check flex mode status
  if (user.flexModeActive && !user.onCooldown) {
    savingsPercent = 0          // Skip savings
    setFlexModeCooldown(7days)  // Start 7-day cooldown
  } else {
    savingsPercent = user.savingsPercentage
  }
  
  // 2. Enforce minimum 10% savings
  if (savingsPercent < 10) {
    savingsPercent = 10
  }
  
  // 3. Calculate split
  savingsAmount = amountUSD * (savingsPercent / 100)
  spendableAmount = amountUSD - savingsAmount
  
  // 4. Convert to Naira
  exchangeRate = getExchangeRate()  // Default: 1 USD = ₦1200
  spendableNaira = spendableAmount * exchangeRate
  ...
  // 6. Return results
  return {
    savingsUSD: savingsAmount,
    spendableNaira,
    exchangeRate,
    flexModeUsed: (savingsPercent === 0)
  }
}
```
...
### SmartWallet.sol
**Key Functions:**
- `splitFunds(user, amount)` - Core split logic
- `activateFlexMode(user)` - Enable savings skip
- `updateSavingsPercentage(user, %)` - Change savings rate
- `withdrawSavings(user, amount)` - Withdraw from vault

**Events:**
- `Deposit` - Fund received and split
- `FlexModeActivated` - Flex mode enabled
- `Withdrawal` - Savings withdrawn
...
```typescript
{
  ...
  walletAddress: string   // Generated (40 hex chars)
  nairaBalance: number    // ₦ spendable
  savingsBalance: number  // USDC vault
  savingsPercentage: number  // 10-100%
  flexModeActive: boolean // Is flex mode ready?
...
```typescript
{
  ...
  amountNaira: number     // Calculated Naira
  savingsAmount: number   // USD saved
  savingsPercentage: number  // % rate applied
  flexModeUsed: boolean   // Was flex mode active?
...
```typescript
const MOCK_EXCHANGE_RATE = 1200  // 1 USD = ₦1200

export function getExchangeRate(): number {
  // Add variation to simulate real market
  return MOCK_EXCHANGE_RATE + (Math.random() * 20 - 10)
  // Returns: 1190-1210 NGN per USD
}
```

**Conversions Applied To:**
- Spendable balance (USD → Naira)
...
1. Get Monad testnet RPC
2. Set private key in `.env`
3. Run: `forge script script/Deploy.s.sol --broadcast`
...
- [x] Simulate transfer → See split
- [x] Activate flex mode → Skip savings
- [x] View insights → See charts
- [x] Change settings → Update percentage

### Edge Cases
- [x] Savings minimum enforcement (10%)
- [x] Flex mode cooldown validation
...
- [ ] Recurring transfers
- [ ] Savings goals with notifications
...
2. **Exchange Rate**: Mock rate (not real-time)
3. **Blockchain**: Contracts not deployed (code only)
4. **Google OAuth**: UI only (not implemented)
5. **Notifications**: UI only (not integrated)
6. **Withdrawal**: Simulated (no actual transfers)
